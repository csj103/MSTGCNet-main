...
class Exp_Anomaly_Detection(Exp_Basic):
    ...
    @staticmethod
    def _loss_reduction(args):
        return getattr(args, "dtsgad_loss_reduction", "sum")

    @classmethod
    def _full_window_loss(cls, values, args):
        if cls._loss_reduction(args) == "mean":
            return values.mean()
        return values.flatten(start_dim=1).sum(dim=1).mean()

    @classmethod
    def _last_step_loss(cls, values, args):
        last_values = values[:, -1, :]
        if cls._loss_reduction(args) == "mean":
            return last_values.mean()
        return last_values.sum(dim=-1).mean()
    ...
    def _model_loss_from_output(cls, model_output, targets, args, loss_mask=None):
        ...
        if "observation_score" in model_output:
            ...
            if loss_mask is None:
                nll_loss = cls._full_window_loss(observation_score, args)
                last_loss = cls._last_step_loss(observation_score, args)
            else:
            ...
            squared_error = (reconstruction - targets).pow(2)
            if loss_mask is None:
                nll_loss = cls._full_window_loss(squared_error, args)
                last_loss = cls._last_step_loss(squared_error, args)
            else:
