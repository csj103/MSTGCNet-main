...
def test_dtsgad_observation_loss_uses_legacy_sum_by_default():
    args = build_args(
        [
            "--dynamic_loss_weight",
            "0.0",
    ...
    assert parts["nll_loss"] == 18.0
    assert parts["last_loss"] == 11.0
    assert torch.allclose(loss, torch.tensor(29.0))


def test_a4_observation_loss_uses_full_and_last_means():
    args = build_args(
        [
    ...
    assert parts["nll_loss"] == 4.5
    assert parts["last_loss"] == 5.5
    assert torch.allclose(loss, torch.tensor(10.0))


def test_a4_loss_reduction_changes_checkpoint_setting():
    base_args = build_args(["--implementation_tag", "dtsgad_a4_last_loss_v1"])
    a4_args = build_args(
        [
            "--implementation_tag",
            "dtsgad_a4_last_loss_v1",
            "--dtsgad_loss_reduction",
            "mean",
            "--last_loss_weight",
            "1.0",
    ...
    assert build_setting(base_args, 0) != build_setting(a4_args, 0)
