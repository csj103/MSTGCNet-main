...
- Eq. (25)-(31): binary-gate balance loss and ATSSD with `L=96`, `alpha=0.01`.
...
- Eq. (10) maps a `dm x L` tensor to `K` weights without defining the temporal
  reduction; this implementation uses temporal mean pooling.
- Eq. (15) does not disclose the internal MHA projection. This implementation
  projects scalar timestamps to `dm`, applies shared MHA within each patch, and
  projects back to the patch feature dimension.
- Fig. 7 visualizes first-block adjacency matrices for patch sizes
  `[8, 12, 16, 32]`, but it does not explicitly declare the complete patch
  assignment of any block. All per-block defaults remain explicit assumptions
  selected from the reported patch pool.
- The activation in Eq. (21), balance coefficient `lambda`, padding modes, and
  several initialization details are not reported.
- The current implementation has about 0.261M trainable parameters, while
